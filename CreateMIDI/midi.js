...
var HDR_SPEED       = "\x03\xe8"; // Defaults to 128 ticks per beat
...
MidiEvent.createNote = function(note, sustained, subtype) {
    ...
    var events = [];
    if (subtype=="noteOn"){
        events.push(MidiEvent.noteOn(note,sustained));
    } else if (subtype=="noteOff"){
        events.push(MidiEvent.noteOff(note,sustained));
    }
    /*
    if (!sustained)
...
var MetaEvent = function(params) {
    if (params &&
        (params.type    !== null || params.type    !== undefined) &&
        (params.data !== null || params.data !== undefined)) {
        this.setType(params.type);
...
MidiTrack.prototype = {
    ...
    setTempo: function(tempo) {
        ...
        var data=[];
        data.push(0x03);
        data.push("0x"+tmp.substr(0,2));
        data.push("0x"+tmp.substr(2,2));
        data.push("0x"+tmp.substr(4,2));
        this.addEvent(new MetaEvent({ type: META_TEMPO, data: data }));
    },
    setTimeSig: function() {
        // TBD
    },
